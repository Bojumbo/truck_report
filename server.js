...
async function init() {
  await pool.query(`CREATE TABLE IF NOT EXISTS users (
    ...
    truck_name TEXT NOT NULL, truck_number TEXT NOT NULL, trailer_name TEXT, trailer_type TEXT NOT NULL, opened_at TIMESTAMPTZ NOT NULL DEFAULT now(), closed_at TIMESTAMPTZ,
    opening_odometer NUMERIC NOT NULL, closing_odometer NUMERIC, created_at TIMESTAMPTZ NOT NULL DEFAULT now()
...
app.post('/api/trips', auth(['driver']), async (req,res) => { const t=req.body; if (!t.truckName||!t.truckNumber||!t.trailerType||t.openingOdometer==null) return res.status(400).json({error:'Заповніть обов’язкові поля'}); try { const {rows}=await pool.query('INSERT INTO trips(driver_id,truck_name,truck_number,trailer_name,trailer_type,opening_odometer) VALUES($1,$2,$3,$4,$5,$6) RETURNING *',[req.user.id,t.truckName,t.truckNumber,t.trailerName||null,t.trailerType,t.openingOdometer]); res.status(201).json(rows[0]); } catch(e){res.status(e.code==='23505'?409:500).json({error:e.code==='23505'?'Активний рейс уже існує':'Не вдалося відкрити рейс'});} });
